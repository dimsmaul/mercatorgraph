...
via a single MCP server and to developers via a view app. Mercatorgraph wraps
...
| [04-services.md](04-services.md) | What worker / mcp / view each do and **why they're separate** |
...
Developer (browser)         ──► View app    ──► generated pages       (optional, humans only)
...
- **Agents only ever talk to the MCP server** — bearer token, scoped per project. They never
  touch the view app and never get the whole graph (every result is size-capped).
...
| **view** | Next.js (Fumadocs) | Human-browsable pages of the graph | Developers (browser) |
...
(agents). Add **view** only if you want the human browse UI.
